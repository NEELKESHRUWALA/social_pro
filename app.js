        ...
        try {
            ...
            if (platform === 'facebook') {
                ...
                if (selectedFiles.length > 0) {
                    ...
                    throw new Error("Instagram requires a media file.");
                }

            } else if (platform === 'youtube') {
                endpoint = '/yt/upload';
                const clientJson = document.getElementById('yt-client-json').files[0];
                if (!clientJson) throw new Error("YouTube requires a Client Secret JSON file.");
                
                formData.append('video', selectedFiles[0]);
                formData.append('client_json', clientJson);
                formData.append('title', caption.substring(0, 100));
                formData.append('description', fullCaption);
                formData.append('category_id', settings.ytCategory || '22');
                formData.append('tags', hashtags.replace(/#/g, ''));
